...
public:
    ...
    void GetState(int player, _Out_ State& state, DeadZone deadZoneMode)
    {
        ...
        if (player >= 0 && player < MAxPLAYER_COUNT)
        {
        ...
        else if (player == c_MostRecent)
        {
            ...
            assert(player >= 0 && player < MAxPLAYER_COUNT);
            device = mInputDevices[player].Get();
            if (!device)
                return;
    ...
    {
        for (int player = 0; player < MAxPLAYER_COUNT; ++player)
        {
            IGameInputDevice* device = mInputDevices[player].Get();
            if (device)
            {
...
private:
    ...
    ComPtr<IGameInputDevice>    mInputDevices[MAxPLAYER_COUNT];
        ...
        _In_ GameInputDeviceStatus) noexcept
    {
        ...
        {
            size_t empty = MAxPLAYER_COUNT;
            size_t k = 0;
            for (; k < MAxPLAYER_COUNT; ++k)
            {
                ...
                {
                    if (empty >= MAxPLAYER_COUNT)
                        empty = k;
                }
            }

            if (k >= MAxPLAYER_COUNT)
            {
        ...
        {
            for (size_t k = 0; k < MAxPLAYER_COUNT; ++k)
            {
                if (impl->mInputDevices[k].Get() == device)
                {
...
public:
    ...
    ~Impl()
    {
        ...
        for (size_t j = 0; j < MAxPLAYER_COUNT; ++j)
        {
            if (mGamePad[j])
            {
...
private:
    ...
    ComPtr<ABI::Windows::Gaming::Input::IGamepad> mGamePad[MAxPLAYER_COUNT];
    EventRegistrationToken mUserChangeToken[MAxPLAYER_COUNT];
...
public:
        ...
        mChanged{}
    {
        ...
        ThrowIfFailed(GetActivationFactory(HStringReference(RuntimeClass_Windows_XboxInput_Gamepad).Get(), mStatics.GetAddressOf()));

        ThrowIfFailed(GetActivationFactory(HStringReference(RuntimeClass_Windows_XboxInput_Controller).Get(), mStaticsCtrl.GetAddressOf()));
...
private:
    ...
    ComPtr<ABI::Windows::Xbox::Input::IGamepad> mGamePad[MAxPLAYER_COUNT];
...
static_assert(GamePad::MAxPLAYER_COUNT == XUSER_MAX_COUNT, "xinput.h mismatch");
