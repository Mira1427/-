...
/*
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE
     ...
     // (In the examples/ app this is usually done within the ImGui_ImplXXxInit() function from one of the demo Renderer)
     int width, height;
     ...
     while (true)
     {
        ...
        // (In the examples/ app this is usually done within the ImGui_ImplXXxNewFrame() function from one of the demo Platform bindings)
 ...
 HOW A SIMPLE RENDERING FUNCTION MAY LOOK LIKE
    ...
    void void MyImGuiRenderFunction(ImDrawData* draw_data)
    {
       ...
       {
          const ImDrawVert* vtxbuffer = cmd_list->VtxBuffer.Data;  // vertex buffer generated by ImGui
          const ImDrawIdx* idxbuffer = cmd_list->IdxBuffer.Data;   // index buffer generated by ImGui
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 MyEngineDrawIndexedTriangles(pcmd->ElemCount, sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, idxbuffer, vtxbuffer);
             }
 ...
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to get anti-aliasing for a marginal cost.
                     ...
                     - the signature of the io.RenderDrawListsFn handler has changed!
                       old: ImGui_XXXxRenderDrawLists(ImDrawList** const cmd_lists, int cmd_lists_count)
                       new: ImGui_XXXxRenderDrawLists(ImDrawData* draw_data).
                         argument:   'cmd_lists' becomes 'draw_data->CmdLists', 'cmd_lists_count' becomes 'draw_data->CmdListsCount'
                         ImDrawList: 'commands' becomes 'CmdBuffer', 'vtxbuffer' becomes 'VtxBuffer', 'IdxBuffer' is new.
                         ImDrawCmd:  'vtxcount' becomes 'ElemCount', 'clip_rect' becomes 'ClipRect', 'user_callback' becomes 'UserCallback', 'texture_id' becomes 'TextureId'.
 ...
 - 2014/08/30 (1.09) - moved IMGUI_FONT_TExUV_FOR_WHITE preprocessor define to IO.FontTexUvForWhite
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    //      glDrawElements(GL_TRIANGLES, (GLsizei)pcmd->ElemCount, sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, idxbuffer_offset);
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
    ...
    if (!(flags & ImGuiWindowFlags_NoSavedSettings))
        if (ImGuiWindowSettings* settings = ImGui::FindWindowSettings(window->ID))
        {
            ...
            window->SettingsIdx = g.SettingsWindows.indexfrom_pointer(settings);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    bool window_size_xset_by_api = false, window_size_y_set_by_api = false;
    ...
    {
        window_size_xset_by_api = (window->SetWindowSizeAllowFlags & g.NextWindowData.SizeCond) != 0 && (g.NextWindowData.SizeVal.x > 0.0f);
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (window_just_created && (!window_size_xset_by_api || !window_size_y_set_by_api))
        ...
        else if (window->AutoFitFramesX > 0 || window->AutoFitFramesY > 0)
        {
            ...
            if (!window_size_xset_by_api && window->AutoFitFramesX > 0)
        ...
        if (!window->Collapsed)
        {
            ...
            float size_xfor_scrollbars = size_full_modified.x != FLT_MAX ? window->SizeFull.x : window->SizeFullAtLastBegin.x;
            float size_y_for_scrollbars = size_full_modified.y != FLT_MAX ? window->SizeFull.y : window->SizeFullAtLastBegin.y;
            window->ScrollbarY = (flags & ImGuiWindowFlags_AlwaysVerticalScrollbar) || ((window->SizeContents.y > size_y_for_scrollbars) && !(flags & ImGuiWindowFlags_NoScrollbar));
            window->ScrollbarX = (flags & ImGuiWindowFlags_AlwaysHorizontalScrollbar) || ((window->SizeContents.x > size_xfor_scrollbars - (window->ScrollbarY ? style.ScrollbarSize : 0.0f)) && !(flags & ImGuiWindowFlags_NoScrollbar) && (flags & ImGuiWindowFlags_HorizontalScrollbar));
...
float ImGui::GetColumnOffset(int column_index)
{
    ...
    const float xoffset = ImLerp(columns->MinX, columns->MaxX, t);
    return xoffset;
}
    ...
    struct Funcs
    {
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                ImDrawIdx* idxbuffer = (draw_list->IdxBuffer.Size > 0) ? draw_list->IdxBuffer.Data : NULL;
                ...
                if (show_draw_cmd_clip_rects && ImGui::IsItemHovered())
                {
                    ...
                    for (int i = elem_offset; i < elem_offset + (int)pcmd->ElemCount; i++)
                        vtxs_rect.Add(draw_list->VtxBuffer[idxbuffer ? idxbuffer[i] : i].pos);
                ...
                while (clipper.Step())
                    for (int prim = clipper.DisplayStart, vtxi = elem_offset + clipper.DisplayStart*3; prim < clipper.DisplayEnd; prim++)
                    {
                        char buf[300];
                        char *buf_p = buf, *buf_end = buf + IM_ARRAYSIZE(buf);
                        ImVec2 triangles_pos[3];
                        for (int n = 0; n < 3; n++, vtxi++)
                        {
                            ImDrawVert& v = draw_list->VtxBuffer[idxbuffer ? idxbuffer[vtxi] : vtxi];
                            triangles_pos[n] = v.pos;
                            buf_p += ImFormatString(buf_p, (int)(buf_end - buf_p), "%s %04d: pos (%8.2f,%8.2f), uv (%.6f,%.6f), col %08X\n", (n == 0) ? "vtx" : "   ", vtxi, v.pos.x, v.pos.y, v.uv.x, v.uv.y, v.col);
                        }
