...
namespace ImGui
{
    ...
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* const items[], int items_count, int popup_maxheight_in_items = -1);
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_maxheight_in_items = -1);      // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    IMGUI_API bool          Combo(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int popup_maxheight_in_items = -1);
    ...
    IMGUI_API void          LogToTTY(int maxdepth = -1);                                       // start logging to tty
    IMGUI_API void          LogToFile(int maxdepth = -1, const char* filename = NULL);         // start logging to file
    IMGUI_API void          LogToClipboard(int maxdepth = -1);                                 // start logging to OS clipboard
...
public:
    ...
    inline int          indexfrom_pointer(const_iterator it) const     { IM_ASSERT(it >= Data && it <= Data+Size); const ptrdiff_t off = it - Data; return (int)off; }
};
...
struct ImDrawCmd
{
    unsigned int    ElemCount;              // Number of indices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtxbuffer[] array, indices in idxbuffer[].
...
struct ImDrawList
{
    ...
    IMGUI_API void  PathArcToFast(const ImVec2& centre, float radius, int a_min_of_12, int a_maxof_12);                                            // Use precomputed angles for a 12 steps circle
    ...
    IMGUI_API void  PrimReserve(int idxcount, int vtxcount);
...
//  - Call SetTexID(my_texid); and pass the pointer/identifier to your texture in a format natural to your graphics API. 
...
struct ImFont
{
    ...
    IMGUI_API ImVec2            CalcTextSizeA(float size, float maxwidth, float wrap_width, const char* text_begin, const char* text_end = NULL, const char** remaining = NULL) const; // utf8
