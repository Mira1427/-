...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (ctx->Map(g_pVB, 0, D3D11_MAP_WRITE_DISCARD, 0, &vtxresource) != S_OK)
        return;
    if (ctx->Map(g_pIB, 0, D3D11_MAP_WRITE_DISCARD, 0, &idxresource) != S_OK)
        return;
    ...
    // Our visible imgui space lies from draw_data->DisplayPos (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). 
    {
        ...
        if (ctx->Map(g_pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) != S_OK)
            return;
        VERTExCONSTANT_BUFFER* constant_buffer = (VERTExCONSTANT_BUFFER*)mapped_resource.pData;
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        {
            D3D11_BUFFER_DESC desc;
            desc.ByteWidth = sizeof(VERTExCONSTANT_BUFFER);
            desc.Usage = D3D11_USAGE_DYNAMIC;
