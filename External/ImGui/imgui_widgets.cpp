...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    else
    {
        ...
        int popup_maxheight_in_items = -1;
        if (flags & ImGuiComboFlags_HeightRegular)     popup_maxheight_in_items = 8;
        else if (flags & ImGuiComboFlags_HeightSmall)  popup_maxheight_in_items = 4;
        else if (flags & ImGuiComboFlags_HeightLarge)  popup_maxheight_in_items = 20;
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_maxheight_in_items)));
    }
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_maxheight_in_items)
{
    ImGuiContext& g = *GImGui;
    ...
    // The old Combo() API exposed "popup_maxheight_in_items". The new more general BeginCombo() API doesn't have/need it, but we emulate it here.
    if (popup_maxheight_in_items != -1 && !g.NextWindowData.SizeConstraintCond)
        SetNextWindowSizeConstraints(ImVec2(0,0), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_maxheight_in_items)));
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (is_power && v_min * v_max < 0.0f)
    {
        ...
        const FLOATTYPE linear_dist_maxto_0 = ImPow(v_max >= 0 ? (FLOATTYPE)v_max : -(FLOATTYPE)v_max, (FLOATTYPE)1.0f/power);
        linear_zero_pos = (float)(linear_dist_min_to_0 / (linear_dist_min_to_0 + linear_dist_maxto_0));
    }
    else
    {
        // Same sign
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_osxshift_shortcut = is_osx && io.KeySuper && io.KeyShift && !io.KeyCtrl && !io.KeyAlt;
        ...
        const bool is_redo  = ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_Y)) || (is_osxshift_shortcut && IsKeyPressedMap(ImGuiKey_Z))) && is_editable && is_undoable;
    ...
    const int buf_display_maxlength = 2 * 1024 * 1024;
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
        ...
        if (is_multiline || buf_display_len < buf_display_maxlength)
    ...
    else
    {
        ...
        if (is_multiline || (buf_end - buf_display) < buf_display_maxlength)
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    bool value_changed_fixhue_wrap = false;
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_RGB || (flags & ImGuiColorEditFlags__InputsMask) == 0)
            if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_RGB))
            {
                ...
                value_changed_fixhue_wrap = (g.ActiveId != 0 && !g.ActiveIdAllowOverlap);
                value_changed = true;
            }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    float maxx = (flags & ImGuiSelectableFlags_SpanAllColumns) ? GetWindowContentRegionMax().x : GetContentRegionMax().x;
    float w_draw = ImMax(label_size.x, window->Pos.x + maxx - window_padding.x - window->DC.CursorPos.x);
    ...
    {
        PushColumnClipRect();
        bb.Max.x -= (GetContentRegionMax().x - maxx);
    }
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    const float y = tab_bar->BarRect.Max.y - 1.0f;
    {
        ...
        const float separator_maxx = tab_bar->BarRect.Max.x + ((flags & ImGuiTabBarFlags_DockNodeIsDockSpace) ? 0.0f : window->WindowPadding.x);
        window->DrawList->AddLine(ImVec2(separator_min_x, y), ImVec2(separator_maxx, y), col, 1.0f);
    }
    return true;
}
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    {
        const float tab_maxwidth = TabBarCalcMaxTabWidth();
        for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
        {
            ImGuiTabItem* tab = &tab_bar->Tabs[tab_n];
            tab->Width = ImMin(tab->WidthContents, tab_maxwidth);
        }
    }
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    tab_bar->LastTabItemIdx = (short)tab_bar->Tabs.indexfrom_pointer(tab);
